...
//! speech-assertion work this file's own history is about.
//!
...
use verbatim_e2e::scenario::is_audible;
...
/// pair per mode — see this file's module doc for why both modes have a
/// known, literal pair rather than a value captured at runtime. `cargo
/// xtask vm test` (audible by default now) always deploys `OneCore` with
/// "Microsoft David" as the default voice and "Microsoft Zira" listed next
/// (confirmed live); a non-audible run (runner-direct with
/// `VERBATIM_E2E_AUDIBLE` unset) uses the capture synth's two fixed names
/// instead.
...
fn m1_exit_criteria_menu_and_speech_dialog() {
    ...
    // rather than a value captured at runtime — see this file's module doc
    // for why both modes have a known, literal pair. This first utterance
    ...
    // and this test must not have to change when that value does.
    ...
    // exposes at all (this file's module doc). Rather than assume a fixed
    ...
    // and points at something deeper than this test's pace, somewhere in
    // the real `OneCore`/`WasapiSink` audio path (outside this crate). This
    // pause stays as a genuine, cheap improvement for borderline cases; the
    // underlying audible-mode gap is unresolved and tracked separately, not
    // fixed by this test.

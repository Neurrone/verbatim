...
- **Sandboxed Wasm extension system**: Replace the broad Python add-on surface with a minimal, deliberately grown host API covering tree queries, event subscriptions, speech/braille output, configuration, and per-extension storage. App modules (Office, Terminal, browsers) live in this iteration-friendly layer. NVDA add-on binary compatibility is a non-goal, though important add-ons should be portable.
